...
<%@ attribute name="ontextchange" type="java.lang.String" required="false" description="id onpropertychange"%>
<div class="input-append">
	<input id="${id}Id" name="${name}"  class="${cssClass}" type="hidden" value="${value}"/>
	<input id="${id}Name" name="${labelName}" ${allowInput?'':'readonly="readonly"'} type="text" value="${labelValue}" data-msg-required="${ not empty dataMsgRequired ? dataMsgRequired:'必填字段'}"
	...
	$("#${id}Button, #${id}Name").click(function(){
	   ...
	   // alert(parentId);
		...
		/* top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, { */
			top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&pId="+parentId+"&isAll=${isAll}", "选择${title}", 300, 420, {
